...
07/24/2026      detolle (Doug Tolley)   INC30712391 — Fix SwapWeight understatement on lots that carry MULTIPLE unwind records.
                                        See "INC30712391 FIX NOTES" block below. *** MUST be deployed together with the
                                        companion change to vw_curr_InTraderOps2 / vw_hist_InTraderOps2 (de-duplicate the
                                        SUM(BookValueAdjHedge)). Deploying this SP alone will break the report the other way. ***
*************************************************************/

/*=============================================================================
  INC30712391 FIX NOTES  —  detolle (Doug Tolley), 2026-07-24 10:14 EDT
  -----------------------------------------------------------------------------
  SYMPTOM
      Portfolio 100AUST BookValueAdjustment understated by $358,835.58 on the
      9 lots of CUSIP 91282CAV3 — a new pay-fixed hedge (HedgeId 519,
      hypo 3208742) booked on lots that still carry unwind amortization from a
      prior hedge. IPOPS2 shows BookValueAdjHedge + BookValueAdjUnwind more
      negative than BookValueAdjustment by exactly that amount.

  ROOT CAUSE
      When a lot has more than one unwind record, PoolMappingsEnrichedFinal
      (below) fans out to ONE ROW PER unwind record (2 rows/lot in this case).
      The SwapWeights CTE then does SUM(bndhold.CurrentFace) over those
      duplicated rows, so TotalCurrentFace is DOUBLED. That halves the
      SwapWeight (1/18 instead of 1/9), and every column weighted by
      (CurrentFace / TotalCurrentFace) — most importantly BookValueHedged on
      line ~751 — absorbs only HALF of HypoMTM.
      Calypso/GL are correct (HypoMTM = -717,671.10); HAT applied only
      -358,835.58. Confirmed against the DB (9.1/9.3 result sets).

  THE FIX IN THIS SP  (SwapWeights CTE, below)
      Make SwapWeights count each lot's CurrentFace exactly ONCE by selecting
      DISTINCT (PoolId, CurrentTicket) from the fanned-out mapping set before
      summing. TotalCurrentFace and SwapWeight then come out correct (1/9),
      which in turn corrects BookValueHedged, BookValueAdjustment, and the
      other SwapWeight-weighted columns (lines ~749, 751, 752-757).

  *** REQUIRED COMPANION CHANGE — DO NOT DEPLOY THIS SP ALONE ***
      The fan-out still leaves 2 rows per lot in ##AdjustedValues (that is by
      design for the multiple-unwind feature — each row carries its own
      FK_ip_accret / FK_ip_amort so the report can break out each unwind).
      The report views vw_curr_InTraderOps2 / vw_hist_InTraderOps2 SUM the
      per-row BookValueAdjHedge across those rows. Today that SUM cancels the
      halved SwapWeight and coincidentally shows the correct hedge; once this
      SP makes SwapWeight correct, that same SUM will DOUBLE the Hedge column
      (-159,482.48 instead of -79,741.24 per lot). The companion view change
      must take BookValueAdjHedge ONCE per ticket (e.g. MAX) while STILL
      SUMming BookValueAdjUnwind. Deploy the SP and the views together.

  ALTERNATIVE (not taken — larger/riskier): remove the fan-out entirely so
      there is one row per lot-hedge with a summed unwind. That would also
      require reworking how the views source BookValueAdjUnwind, so the
      surgical CTE fix above was chosen instead.
=============================================================================*/
CREATE PROCEDURE [dbo].[spInsertValsToAdjValTemp_FIX]
...
--
-- REMINDER: pair with the vw_curr_InTraderOps2 / vw_hist_InTraderOps2 change that
--   takes BookValueAdjHedge ONCE per ticket (see FIX NOTES at top).
-- ==================================================================================
