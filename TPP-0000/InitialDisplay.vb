    ...
    Private Sub SetEnvironmentColor()
        Try
            ...
            Select Case CommonUI.Connector.Environment
                Case "Development"
                    colorName = GetSettingOrDefault("DevEnvironmentColor")
                Case "IT"
                    colorName = GetSettingOrDefault("ITEnvironmentColor")
                Case "UAT"
                    colorName = GetSettingOrDefault("UATEnvironmentColor")
                Case "Parallel"
                    colorName = GetSettingOrDefault("ParallelEnvironmentColor")
                Case Else
            ...
            If Not String.IsNullOrEmpty(colorName) Then
                Dim envColor As Color = Color.FromName(colorName)
                'Color.FromName returns a color with IsKnownColor=False and ARGB=0 if invalid
                If envColor.IsKnownColor OrElse (envColor.A <> 0 OrElse envColor.R <> 0 OrElse envColor.G <> 0 OrElse envColor.B <> 0) Then
